...
class XFluxPipeline:
    ...
    def set_controlnet(self, control_type: str, local_path: str = None, repo_id: str = None, name: str = None):
        ...
        checkpoint = load_checkpoint(local_path, repo_id, name)
        self.controlnet.load_state_dict(checkpoint, strict=True)
    ...
    def __call__(self,
                 ...
                 controlnet_image: np.ndarray = None,
                 width: int = 512,
                 height: int = 512,
                 guidance: float = 4,
                 num_steps: int = 50,
        ...
        if self.controlnet_loaded:
            ...
            controlnet_image = torch.from_numpy(controlnet_image)  # [h, w]
            controlnet_image = depth_transforms(controlnet_image[None, ...])
            controlnet_image = controlnet_image.clamp(min=0., max=1.)
            controlnet_image = ((controlnet_image - controlnet_image.min()) / (controlnet_image.max() - controlnet_image.min())) * 2 - 1
            controlnet_image = controlnet_image.repeat(3, 1, 1)  # [3, h ,w]
            controlnet_image = controlnet_image.unsqueeze(0).to(torch.bfloat16).to(self.device)
            ...
            depth_cond = apply_depth_colormap(controlnet_image[0][0].float())
            depth_cond.save('results/depth_cond.png')
    ...
    def infer_data(self,data : dict, seed=12345, dtype=torch.float32):
        with torch.autocast(device_type='cuda',dtype=dtype):
            prompt = data['val_prompt']
            depth = data['val_depth']
            width = 480
            height = 320
            guidance = 4.0
            num_steps = 28
            ...
            with torch.no_grad():
                ...
                neg_inp_cond = prepare(t5=self.t5, clip=self.clip, img=x, prompt="")
            ...
            controlnet_image = torch.from_numpy(depth)  # [h, w]
            controlnet_image = depth_transforms(controlnet_image[None, ...])
            if controlnet_image.min() < 1.:
                controlnet_image = controlnet_image.clamp(min=0., max=1.)
            controlnet_image = ((controlnet_image - controlnet_image.min()) / (controlnet_image.max() - controlnet_image.min())) * 2 - 1
            controlnet_image = controlnet_image.repeat(3, 1, 1)  # [3, h ,w]
            controlnet_image = controlnet_image.unsqueeze(0).to(torch.bfloat16).to(self.device)

            x0 = denoise_controlnet(
                    self.model,
                    **inp_cond,
...
class XFluxSampler(XFluxPipeline):
    def __init__(self, clip, t5, ae, model, controlnet, device):
        self.clip = clip
        self.t5 = t5
        self.ae = ae
        self.model = model
